...
-(void)example01_imageAd_networkData
{
    ...
    //广告数据请求
    NSURL*CouPonUrl=[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",BaseDataHeaderC,ADURL]];
    AFHTTPSessionManager*couPonManager=[AFHTTPSessionManager manager];
    couPonManager.responseSerializer=[AFHTTPResponseSerializer serializer];
//    if (User_Message.isLogin) {
//        [couPonManager.requestSerializer setValue:User_Message.userToken forHTTPHeaderField:@"token"];
//    }else{
//        [couPonManager.requestSerializer setValue:@"" forHTTPHeaderField:@"token"];
//    }
    [couPonManager GET:CouPonUrl.absoluteString parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSMutableDictionary *CPmutableDic=[NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableLeaves error:nil];
        NSString*status=CPmutableDic[@"status"];
        if ([status intValue]==1) {
            //广告数据转模型
            LaunchAdModel *model = [LaunchAdModel new];
            model.content = CPmutableDic[@"data"][@"pictureUrl"];
            model.duration = 3;
            model.openUrl = @"";
            model.contentSize = [NSString stringWithFormat:@"%f*%f",SCREENWIDTH,SCREENHEIGHT];
            //配置广告数据
