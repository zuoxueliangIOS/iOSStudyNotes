...
{
    return [[UIScreen mainScreen] bounds].size.height - (frame.origin.y + kChatToolBarHeight);
}
...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    if (translucent) {
        frame = CGRectMake(0, [[UIScreen mainScreen] bounds].size.height - kChatToolBarHeight, [[UIScreen mainScreen] bounds].size.width, kChatKeyBoardHeight);
    }else {
        frame = CGRectMake(0, [[UIScreen mainScreen] bounds].size.height - kChatToolBarHeight - 64, [[UIScreen mainScreen] bounds].size.width, kChatKeyBoardHeight);
    }
...
{
    CGRect frame = CGRectMake(0, bounds.size.height - kChatToolBarHeight, [[UIScreen mainScreen] bounds].size.width, kChatKeyBoardHeight);
    return [[self alloc] initWithFrame:frame];
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _chatToolBar = [[ChatToolBar alloc] initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, kChatToolBarHeight)];
        _chatToolBar.delegate = self;
        [self addSubview:self.chatToolBar];
        
        _facePanel = [[FacePanel alloc] initWithFrame:CGRectMake(0, kChatKeyBoardHeight-kFacePanelHeight, [[UIScreen mainScreen] bounds].size.width, kFacePanelHeight)];
        ...
        _OAtoolbar = [[OfficialAccountToolbar alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.frame), [[UIScreen mainScreen] bounds].size.width, kChatToolBarHeight)];
...
- (void)keyBoardWillChangeFrame:(NSNotification *)notification
{
    ...
    if (self.chatToolBar.faceSelected && (getSupviewH(self.keyboardInitialFrame) - CGRectGetMidY(self.frame)) < CGRectGetHeight(self.frame))
    {
        [UIView animateWithDuration:0.25 delay:0.03 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            self.frame = CGRectMake(0, getSupviewH(self.keyboardInitialFrame)-CGRectGetHeight(self.frame), [[UIScreen mainScreen] bounds].size.width, CGRectGetHeight(self.frame));
    ...
    else
    {
        [UIView animateWithDuration:0.25 animations:^{
            ...
            else if (end.origin.y == [[UIScreen mainScreen] bounds].size.height && begin.origin.y!=end.origin.y && duration > 0)
            {
                //键盘收起
